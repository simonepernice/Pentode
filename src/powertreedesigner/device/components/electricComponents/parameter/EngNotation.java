...
/* I make 2 changes to the original file:
- Fixed the number of decimal places in a constant changeable by user
- Added Local.US in String.format otherwise Java would use , instead of . in Italy
...
public class EngNotation {
    ...
    public static void setDECIMALPLACES(int DECIMALPLACES) {
        EngNotation.DECIMALPLACES = DECIMALPLACES;
    }
    ...
    public static String convert(double val) {
        ...
        if (val == 0) {
            return String.format(Locale.US, "%." + DECIMALPLACES + "f", 0.0);
        }
        ...
        if (index >= 0 && index < PREFIX_ARRAY.length) {
            ...
            return String.format(Locale.US, "%." + DECIMALPLACES + "f%s", val, PREFIX_ARRAY[index]);
        } else {
            // If no prefix exists just make a string of the form 000e000
            return String.format(Locale.US, "%." + DECIMALPLACES + "fe%d", val, count * 3);
        }
    }
    ...
    public static double parse(char[] chars) throws NumberFormatException {
        ...
        for (int c = start; c <= end; c++) {
            ...
            if (chars[c] >= '0' && chars[c] <= '9') {
                ...
                if (gotPrefix && gotDP) {
                    throw new NumberFormatException("Cannot have digits after prefix when number includes decimal point");
                }
                value *= 10;
                value += chars[c] - '0';
                gotDigit = true;
