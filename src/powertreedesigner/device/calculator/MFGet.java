 ...
 */
public class MFGet extends MathFunction {

    public MFGet() {
        super("get", 0, "get the given element from a vector");
    }

    @Override
    public void calculate(Stack<VectorOfDouble> stack) throws CalculationException {
        LinkedList<Double> positions = stack.pop().getData();
        LinkedList<Double> data = stack.pop().getData();
        LinkedList<Double> res = new LinkedList<>();            
        for (double p : positions)
            res.add(data.get((int)p));
